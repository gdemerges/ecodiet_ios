...
struct EcoQuizView: View {
    ...
    init() {
        ...
        self.questions = QuizData.ecoQuestions.shuffled().prefix(5).map { $0 }
    }
    ...
    private var resultFeedback: some View {
        VStack(spacing: 16) {
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 16, style: .continuous)
                    .stroke(isAnswerCorrect ? Color.green.opacity(0.3) : Color.red.opacity(0.3), lineWidth: 2)
            )
    ...
    private var scoreIcon: String {
        ...
        if percentage >= 0.4 { return "face.smiling" }
    ...
    private var scoreMessage: String {
        let percentage = Double(score) / Double(questions.count)
        if percentage >= 0.8 {
            return "Vous êtes un véritable expert de l'alimentation écologique ! Bravo ! 🌱"
        }
        if percentage >= 0.6 {
            return "Vous avez de bonnes connaissances sur l'alimentation durable. Continuez comme ça ! 🌿"
        }
        if percentage >= 0.4 {
            return "Vous êtes sur la bonne voie ! Continuez à vous informer sur l'alimentation écologique. 🌾"
        }
        return "Il y a encore des choses à apprendre, mais chaque geste compte ! 🌍"
    }
